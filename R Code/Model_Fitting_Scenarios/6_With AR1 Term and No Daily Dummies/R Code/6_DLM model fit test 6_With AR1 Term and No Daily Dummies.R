...
out = '/Users/jennstarling/UTAustin/Research/ercot/R Code/Model_Fitting_Scenarios/6_With AR1 Term and No Daily Dummies/Figures/'
...
#Read in data.
y_all = readRDS('R Code/Model_Fitting_Scenarios/6_With AR1 Term and No Daily Dummies/R Data Objects/dlm_y_allzones.rda')
F_all = readRDS('R Code/Model_Fitting_Scenarios/6_With AR1 Term and No Daily Dummies/R Data Objects/dlm_F_allzones.rda')
G_all = readRDS('R Code/Model_Fitting_Scenarios/6_With AR1 Term and No Daily Dummies/R Data Objects/dlm_G_allzones.rda')
datetime_info = readRDS('R Code/Model_Fitting_Scenarios/6_With AR1 Term and No Daily Dummies/R Data Objects/dlm_datetime_info.rda')
...
# #Plot each hour-of-day dummy var over time.
# pdf(paste(out,"07_COAST_Win9_Daily Dummy Trace.pdf"), width=18, height=12)
	
	# days = c("M","T","W","Th","F","Sa")
	# day.cols = which(colnames(F) %in% days)
	
	# par(mfrow=c(3,2),oma=c(1,1,0,0) + .1, mar=c(0,0,1,1)+1)
	# for (i in day.cols){
		# day.label = days[i-day.cols[1]+1]
		# plot(test.ffbs$m[i,],type='l',xlab=day.label)
		# legend('topright',day.label)
	# }
# dev.off()

#--------------------------------------------------------
#   Trace Plots: AR1 Term
#	Plot AR1 variable's trace over time.   
#--------------------------------------------------------
...
pdf(paste(out,"08_COAST_Win9_AR1 Trace.pdf"), width=18, height=12)

	ar1.col = which(colnames(F)=="ar1")
	plot(test.ffbs$m[ar1.col,],type='l')
...
pdf(paste(out,"04_COAST_Win9_Forecast.pdf"), width=12, height=6)
...
pdf(paste(out,"04c_COAST_Win9_Forecast_zoom.pdf"), width=12, height=6)
	par(mfrow=c(1,2))
	
	#Plot scaled actual and predicted for forecast.
	plot(y.future, main='Scaled Forecast')
	lines(y.pred.mean, lwd=2, col='blue')
	
	#Plot re-sccaled actual and predicted for forecast.
	plot(y.future.rescaled, main='Un-Scaled Forecast')
	lines(y.pred.mean.rescaled, lwd=2, col='blue')
...
system('say DLM script 6 has completed!')
