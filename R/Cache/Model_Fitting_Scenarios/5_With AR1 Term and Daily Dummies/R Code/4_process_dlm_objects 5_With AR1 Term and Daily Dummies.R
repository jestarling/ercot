...
#Library Hmsc for lagging AR1 observations.
library(Hmisc)
...
### Peel off first values, as we will need a lag-1 load term y0, so must use our first
### observation as this lag-1.  So our data will start at our second obs.

#Save AR1 values for F matrix.  (Excludes last term.)
ar1 = load[-nrow(load),]

#Update load, bushr and zone_temp to exclude 1st row, since 1st obs is used as AR1 initial term.
load = load[-1,]
zone_temp = zone_temp[-1,]
bushr = bushr[-1,]
...
p = 34			#Num predictors - adjusted to add day-of-week dummy variables and autoregressive term.
...
for (i in 1:8){
	...
	#------------------------------------------------------------
	# Add AR1 auto-regressive term (y_{t-1}) to F matrix.
	#------------------------------------------------------------
	...
	F[[i]] = as.matrix(cbind.data.frame(F.int,F.temp,F.temp2,F.holiday,ar1=F.ar1,F.hrdummies,F.daydummies))
}
...
#Output R objects.
saveRDS(y_all,'R Code/Model_Fitting_Scenarios/5_With AR1 Term and Daily Dummies/R Data Objects/dlm_y_allzones.rda')
saveRDS(F_all,'R Code/Model_Fitting_Scenarios/5_With AR1 Term and Daily Dummies/R Data Objects/dlm_F_allzones.rda')
saveRDS(G_all,'R Code/Model_Fitting_Scenarios/5_With AR1 Term and Daily Dummies/R Data Objects/dlm_G_allzones.rda')
saveRDS(datetime_info,'R Code/Model_Fitting_Scenarios/5_With AR1 Term and Daily Dummies/R Data Objects/dlm_datetime_info.rda')
