...
p = 200			#Num predictors. WITH DAILY #****************
...
for (i in 1:8){
	...
	#------------------------------------------------------------
	# Add Day*Hr interaction to F matrix.  Sunday.0 is baseline.
	#------------------------------------------------------------
	
	#Set up interaction columns.
	int.cols = expand.grid(hrs = as.character(0:23), days=days.list)[-1,] #Sunday, Hour 0 is baseline.
	F.intdummies = matrix(0, nrow=n, ncol=nrow(int.cols)) 
		
	for (j in 1:nrow(int.cols)){
		F.intdummies[ which(wk.days==int.cols[j,2] & as.numeric(substr(load$Time,12,13))==int.cols[j,1]) , j] = 1
	}
	
	#Set up interaction column names.
	int.colnames = expand.grid(hrs = as.character(0:23), days=c('S','M','T','W','Th','F','Sa'))
	int.colnames = apply( int.colnames[, c('days','hrs')], 1, paste, collapse = ".")[-1] #Remove first entry; baseline.
	
	colnames(F.intdummies) = int.colnames
	...
	F[[i]] = as.matrix(cbind.data.frame(F.int,F.temp,F.temp2,F.holiday,F.hrdummies,F.daydummies,F.intdummies)) #***********
...
#Output R objects.
saveRDS(y_all,'R Code/Model_Fitting_Scenarios/7_With Daily Dummies and Day_x_Hr Interaction/R Data Objects/dlm_y_allzones.rda')
saveRDS(F_all,'R Code/Model_Fitting_Scenarios/7_With Daily Dummies and Day_x_Hr Interaction/R Data Objects/dlm_F_allzones.rda')
saveRDS(G_all,'R Code/Model_Fitting_Scenarios/7_With Daily Dummies and Day_x_Hr Interaction/R Data Objects/dlm_G_allzones.rda')
saveRDS(datetime_info,'R Code/Model_Fitting_Scenarios/7_With Daily Dummies and Day_x_Hr Interaction/R Data Objects/dlm_datetime_info.rda')
