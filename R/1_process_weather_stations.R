...
weather_data = foreach(i = seq_along(station_list), .combine='rbind') %do% {
	...
	path_name = paste0('DataRaw/weather_stations/', station_list[i])
...
temp = temperature_scrub	#Set as a separate variable so can easily bypass scrubbing later if needed.
...
dim(dewpoint_impute)
dim(windSpeed_impute)

#There are 1105 more weather times than load times.
#These are tacked on the end.  We will leave these alone, for forecasting purposes, for now.
